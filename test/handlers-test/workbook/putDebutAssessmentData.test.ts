...
describe("putDebutAssessmentData", () => {
  ...
  it("returns a promise", () => {
    ...
    expect(
      putDebutAssessmentData(candidateId, workflowId, workflow)
  ...
  it("calls putItem with the correct params", async () => {
    ...
    const putItemSpy = jest.spyOn(putItemMod, "putItem");
    putItemSpy.mockResolvedValueOnce("dummy_put_item_response" as any);
    await putDebutAssessmentData(candidateId, workflowId, workflow).then(
      (x) => {
  ...
  it("throws an error if the putItem call fails", async () => {
    ...
    await putDebutAssessmentData(candidateId, workflowId, workflow).catch(
      (err) => {
